...
{
	// Ambient
	vec3	ambient				= light.ambientFactor * light.color * texture(material.diffuseMap, v_TexCoord).rgb;
	...
	float	shininessFactor		= pow(max(dot(viewDir, reflectDir), 0.0f), material.shininessFactor);
	vec3	specular			= light.specularFactor * light.color * shininessFactor * texture(
									(material.hasSpecularMap ? material.specularMap : material.diffuseMap), v_TexCoord).rgb;

	// Emission
	vec3	emission			= (material.hasEmissionMap ? texture(material.emissionMap, v_TexCoord).rgb : vec3(0.0f, 0.0f, 0.0f));
...
{
	// Ambient
	vec3	ambient				= light.ambientFactor * light.color * material.ambientColor;
	...
	vec3	reflectDir			= reflect(-lightDir, norm);
	float	shininessFactor		= pow(max(dot(viewDir, reflectDir), 0.0f), material.shininessFactor);
	vec3	specular			= light.specularFactor * light.color * shininessFactor * material.specularColor;
